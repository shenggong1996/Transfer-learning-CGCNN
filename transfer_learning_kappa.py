...
#this framework is developed for transfer learning from the calcluated thermal conductivities from TEDesignLab to 
...
#is very small (typically less than 500 data). This framework is based on Crystal Graph Convolutional Neural Networks (CGCNN) 
...
parser.add_argument('--train', default=1, type=int, help='train or test mode')
...
def main():
    ...
    _, crys_fea,target  = validate(test_loader, model, criterion, normalizer, test=True)
#    print (crys_fea, crys_fea.shape, target)
    crys_fea = crys_fea.cpu()
    crys = crys_fea.detach().numpy()
    target = target.cpu()
    target = target.detach().numpy()
    if args.train:
        # set parameters for the last additional layer
        reg = neural_network.MLPRegressor(tol = 1e-4, hidden_layer_sizes = (64), solver = 'lbfgs',alpha = 0.1, verbose = False, activation = 'identity',max_iter=200)
        reg.fit(crys, target)
        pickle.dump(reg, open('last_layer.sav', 'wb'))
        print ('transfer learning finished, model saved')
    else:
        reg = pickle.load(open('last_layer.sav', 'rb'))
        output = reg.predict(crys)
        np.savetxt('results.csv', output, delimiter=',')
        print ('prediction finished, results saved with MAE %f'%(mean_absolute_error(output, target)))
#    np.savetxt('fea.csv', crys, delimiter=',')
...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    if model_args.task == 'regression':
        print(' {star} MAE {mae_errors.avg:.3f}'.format(star=star_label,
                                                        mae_errors=mae_errors))
        return mae_errors.avg, crys_fea, test_target
    else:
        print(' {star} AUC {auc.avg:.3f}'.format(star=star_label,
                                                 auc=auc_scores))
        return auc_scores.avg, crys_fea
